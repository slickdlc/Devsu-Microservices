...
import com.devsu.customer.application.usecase.UpdateCustomerUseCaseImpl;
...
class UpdateCustomerUseCaseImplTest {
  ...
  void given_serviceWorks_when_handle_then_successUseCase() {
    ...
    verify(this.customerService).updateCustomer(CustomerMother.complete());
  }

  @Test
  void given_serviceFails_when_handle_then_throwUseCaseException() {
    doThrow(ServiceException.class).when(this.customerService).updateCustomer(CustomerMother.complete());
