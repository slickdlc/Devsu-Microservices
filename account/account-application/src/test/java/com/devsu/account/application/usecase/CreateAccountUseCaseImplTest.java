...
class CreateAccountUseCaseImplTest {
  ...
  void given_serviceWorks_when_handle_then_successUseCase() {
    ...
    verify(this.accountService).createAccount(AccountMother.complete());
  }

  @Test
  void given_serviceFails_when_handle_then_throwUseCaseException() {
    doThrow(ServiceException.class).when(this.accountService).createAccount(AccountMother.complete());
