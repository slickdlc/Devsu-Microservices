...
class DeleteAccountUseCaseImplTest {
  ...
  void given_serviceWorks_when_handle_then_successUseCase() {
    ...
    verify(this.accountService).deleteAccount(AccountMother.ACCOUNT_ID);
  }

  @Test
  void given_serviceFails_when_handle_then_throwUseCaseException() {
    doThrow(ServiceException.class).when(this.accountService).deleteAccount(AccountMother.ACCOUNT_ID);
